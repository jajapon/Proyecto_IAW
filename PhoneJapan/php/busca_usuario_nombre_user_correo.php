...
$mysqli = new mysqli("localhost","root","","phonejapan");
    ...
    $dato = $_GET["dato"];
    $result = $mysqli->query("SELECT COUNT(*) as conteo FROM usuario WHERE EMAIL LIKE '%$dato%' OR NOMBRE LIKE '%$dato%' OR APELLIDOS LIKE '%$dato%' OR USERNAME LIKE '%$dato%'");
    if($result){
    ...
    $result = $mysqli->query("SELECT * FROM usuario WHERE EMAIL LIKE '%$dato%' OR NOMBRE LIKE '%$dato%' OR APELLIDOS LIKE '%$dato%' OR USERNAME LIKE '%$dato%' LIMIT $limit, $nelementos");
